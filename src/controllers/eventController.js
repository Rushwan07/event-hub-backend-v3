...
exports.createEvent = catchAsync(async (req, res, next) => {
  ...
  const newEvent = new Event({
    ...
    venue: venue ? (typeof venue === "string" ? JSON.parse(venue)?.id : venue.id) : undefined,
    catering: catering
      ? typeof catering === "string"
        ? JSON.parse(catering)?.id
        : catering.id
      : undefined,
    photograph: photograph
      ? typeof photograph === "string"
        ? JSON.parse(photograph)?.id
        : photograph.id
      : undefined,
    decoration: decoration
      ? typeof decoration === "string"
        ? JSON.parse(decoration)?.id
        : decoration.id
      : undefined,
  ...
  for (const service of services) {
    if (service && service.id && service.clientId) {
      const { id, clientId } = service;
