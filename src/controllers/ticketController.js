...
exports.bookTickets = catchAsync(async (req, res, next) => {
  const { eventId, buyerId, tickets } = req.body;

  if (!eventId || !buyerId || !tickets || !Array.isArray(tickets)) {
  ...
  const test = await Promise.all(ticketPromises);
  let totalQuantity = 0,
    totalCost = 0;
  for (t of test) {
...
`;
  ...
  await transporter.sendMail({
    to: user.email,
    subject: `Hello ${user.name}, Thank you for purchasing ${event.name} tickets`,
    html: emailContent,
  });
